function folderpwd = folderselection_10j_aptest(pwd)
    ...
    for i = 1:1:length(subFolderNames)
     ...
     % I have done an string matrix with all the directory location of the
        ...
        i_sn = 1; i_vc = 1; i_sl = 1; i_slREM = 1;
        for i = 1:1:length(epochsubfoldername)
            folderpwd(i,1) = strcat(strdir, '\',epochsubfoldername(i,1));
            if contains(epochsubfoldername(i,1),'sn-ap') == 1
               dirSn(i_sn,1) = folderpwd(i,1);
               snNum(1,i_sn) = extractBefore(dirSn(i_sn,1),'sn-ap');
               snNum(1,i_sn) = extractAfter(snNum(1,i_sn),'epoch');
               i_sn = i_sn + 1;
            elseif contains(epochsubfoldername(i,1),'vc-ap') == 1
               dirVc(i_vc,1) = folderpwd(i,1);
               vcNum(1,i_vc) = extractBefore(dirVc(i_vc,1),'vc-ap');
               vcNum(1,i_vc) = extractAfter(vcNum(1,i_vc),'epoch');
               i_vc = i_vc +1;
           elseif contains(epochsubfoldername(i,1),'sommeil_REM-ap') == 1
               dirSl_REM(i_slREM,1) = folderpwd(i,1);
               slREMNum(1,i_slREM) = extractBefore(dirSl_REM(i_slREM,1),'sommeil_REM-ap');
               slREMNum(1,i_slREM) = extractAfter(slREMNum(1,i_slREM),'epoch');
               i_slREM = i_slREM +1;
           elseif contains(epochsubfoldername(i,1),'sommeil-ap') == 1
               dirSl(i_sl,1) = folderpwd(i,1);
               slNum(1,i_sl) = extractBefore(dirSl(i_sl,1),'sommeil-ap');
        ...
        [~, slREMPos] = sort(str2double(slREMNum));
        ...
        %The sleeping ones, not in REM phase
        ...
        %Finally the sleeping ones in REM phase
        for i = 1:length(slREMNum) 
            folder(i+ i_fin,1) = dirSl_REM(slREMPos(1,i),1);
        end
        folderpwd = folder;
end